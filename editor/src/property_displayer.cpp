...
PropertyDisplayer::PropertyDisplayer(Editor &editor)
  : editor(editor) {}
auto PropertyDisplayer::operator()(kuki::BoneData *boneData) -> void {
  if (!boneData)
    return;
...
auto PropertyDisplayer::operator()(kuki::Camera *camera) -> void {
  ...
  if (camera->type == kuki::CameraType::Perspective) {
    ...
    if (ImGui::SliderFloat("Aspect Ratio", &aspectRatio, .1f, 10.f)) {
...
auto PropertyDisplayer::operator()(kuki::GLMaterial *material) -> void {
  if (!material)
    return;
...
auto PropertyDisplayer::operator()(kuki::GLMesh *mesh) -> void {
  if (!mesh)
    return;
...
auto PropertyDisplayer::operator()(kuki::GLTexture *texture) -> void {
  if (!texture)
    return;
...
auto PropertyDisplayer::operator()(kuki::Light *light) -> void {
  if (!light)
    return;
...
auto PropertyDisplayer::operator()(std::monostate) -> void {}
