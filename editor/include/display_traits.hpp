...
template <>
struct DisplayTraits<GLMaterial> {
  static void DisplayProperties(GLMaterial &material, EditorContext &context) {
    ...
    const auto SelectProperty = [&](const MaterialProperty prop) {
      context.selectedComponentType = ComponentType::TextureHandle;
      context.selectedProperty = static_cast<uint8_t>(prop);
    };
    struct TexEntry {
      const char *id;
      ImTextureID tex;
    ...
    TexEntry entries[] = {
      {"Albedo", static_cast<ImTextureID>(material.textures.albedo), "Albedo", MaterialProperty::AlbedoTexture},
...
template <>
struct DisplayTraits<GLMesh> {
  static void DisplayProperties(GLMesh &mesh, EditorContext &context) {
    if (!mesh)
      return;
...
template <>
struct DisplayTraits<GLSkybox> {
  static void DisplayProperties(GLSkybox &skybox, EditorContext &context) {
    ...
    if (clicked)
      context.selectedComponentType = ComponentType::SkyboxHandle;
...
template <>
struct DisplayTraits<GLTexture> {
  static void DisplayProperties(GLTexture &texture, EditorContext &context) {
